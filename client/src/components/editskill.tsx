...
import { skilllevel, skill } from "../../types/types";
...
const UserSkillEdit: FC = () => {
  ...
  const [editUserSkill, setEditUserSkill] = useState<skill[]>([]);
  useEffect(() => {
    //APIからSkillデータを取得する関数を定義
    const fetchAllSkills = async () => {
      try {
        const res = await axios.get(`${apiUrl}/api/v1/skill/${id}`);
        console.log("res", res);
        setEditUserSkill(res.data);
      } catch (error) {
        console.log(error);
      }
    };
