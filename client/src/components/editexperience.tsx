...
import { experience, experienceCategory, month, year } from "../../types/types";
...
const UserExperienceEdit: FC = ({}) => {
  const router = useRouter();
  const { id } = router.query;
  const [open, setOpen] = useState(false);
  const apiUrl = process.env.NEXT_PUBLIC_API_URL;

  const [experienceCategorys, setExperienceCategorys] = useState<
    experienceCategory[]
  >([]);
  useEffect(() => {
    //APIからcategoryデータを取得する関数を定義
    const fetchAllCategorys = async () => {
      try {
        const res = await axios.get(`${apiUrl}/api/v1/experiencecategory`);
        console.log("res", res);
        setExperienceCategorys(res.data);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  useEffect(() => {
    //APIからyearデータを取得する関数を定義
    const fetchAllYears = async () => {
      try {
        const res = await axios.get(`${apiUrl}/api/v1/year`);
        console.log("res", res);
        setYears(res.data);
      } catch (error) {
        console.log(error);
  ...
  useEffect(() => {
    //APIからmonthデータを取得する関数を定義
    const fetchAllMonths = async () => {
      try {
        const res = await axios.get(`${apiUrl}/api/v1/month`);
        console.log("res", res);
        setMonths(res.data);
      } catch (error) {
        console.log(error);
  ...
  const [editUserExperience, setEditUserExperience] = useState<experience[]>(
    []
  );
  useEffect(() => {
    //APIからExperienceデータを取得する関数を定義
    const fetchAllExperiences = async () => {
      try {
        const res = await axios.get(`${apiUrl}/api/v1/experience/${id}`);
        console.log("res", res);
        setEditUserExperience(res.data);
      } catch (error) {
        console.log(error);
      }
    };
